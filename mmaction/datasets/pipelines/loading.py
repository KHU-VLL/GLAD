...
class MultiScaleSampleFrames:
    def __init__(self,
        ...
        **kwargs
        ...
        self.with_dense_sampler = with_dense_sampler
        ...
        if self.dense_sampling:
            if self.with_dense_sampler:
                self.global_sampler = DenseSampleFrames(
                    clip_len=1,
                    frame_interval=1,
    ...
    def __call__(self, results):
        ...
        results['num_clips'] = self.num_locals + (
            self.global_sampler.num_sample_positions if self.test_mode and isinstance(self.global_sampler, DenseSampleFrames)
            else self.num_globals)
...
@PIPELINES.register_module()
class TOLSampleFrames(SampleFrames):
    def __init__(
        self,
        num_clips=3,
        ...
        TOLSampleFrames) 1111 1    22  22 2    33  33 3
        """
        self.num_clips = num_clips
        self.clip_len = clip_len
...
class DecordInit:
    ...
    def __call__(self, results):
        ...
        #         p_fail = Path(f'tools/data/kinetics/k400/failed/{labelname}')
