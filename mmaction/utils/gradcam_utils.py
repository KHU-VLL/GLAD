...
class GradCAM:
    ...
    def __init__(self, model, target_layer_name, colormap='viridis'):
        ...
        """
        from ..models.recognizers import Recognizer2D, Recognizer3D, TemporallyPyramidicRecognizer, TemporallyPyramidicDARecognizer
        ...
        elif isinstance(model, (TemporallyPyramidicRecognizer, TemporallyPyramidicDARecognizer)):
            from mmaction.models.backbones import ResNetTSM
            index = model.sampler.index_dict
            self.is_multiscale = index['l'] and index['g']  # both not None
            if isinstance(model.backbone, ResNetTSM):
                self.is_recognizer2d = True
            else:
                self.is_recognizer2d = False
        else:
        ...
        if model.cfg.data.test.pipeline[-3]['input_format'] in ['NCTHW', 'TCNHW']:
            self.is_input_2d = False
        else:
            self.is_input_2d = True
    ...
    def _register_hooks(self, layer_name):
        ...
        def get_activations(module, input, output):
            if self.is_multiscale and self.target_activations is not None:
                self.target_activations = torch.concat([self.target_activations, output.clone().detach()])
            else:
                self.target_activations = output.clone().detach()

        def get_gradients(module, grad_input:tuple, grad_output:tuple):  # why tuple? => the # of module's output can be > 1
            if self.is_multiscale and self.target_gradients is not None:
                self.target_gradients = torch.concat([self.target_gradients, grad_output[0].detach()])
            else:
    ...
    def _calculate_localization_map(self, inputs, use_labels, delta=1e-20):
        ...
        preds = self.model(gradcam=True, **inputs)  # [B, K]
        if use_labels:
            labels = inputs['label']
            if labels.ndim == 1:
        ...
        if self.is_input_2d:
            ...
            b = b1 * b2
        # B = batch_size * num_crops * num_clips
        ...
        weights = torch.mean(gradients.view(b, tg, c, -1), dim=3)  # -> [B, Tg, C', H' x W'] -> [B, Tg, C']
        ...
        localization_map = localization_map.permute(0, 2, 1, 3, 4)  # [B, C''=1, Tg, H', W']
        ...
        print(
            '\nShapes:\n',
            f'  {"inputs":15s}', inputs['imgs'].shape, '\n',
            f'  {"grads":15s}', gradients.shape, '\n',
            f'  {"grad-weights":15s}', weights.shape, '\n',
            f'  {"activations":15s}', activations.shape, '\n',
            f'  {"map":15s}', localization_map.shape, '\n',
            sep=''
        )
